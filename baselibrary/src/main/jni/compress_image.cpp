...
METHODDEF(void) error_exit(j_common_ptr info) {
    ...
    LOG_E("jpeg error_exit,jpeg_message_table[%d]:%s", err->pub.msg_code,
          err->pub.jpeg_message_table[err->pub.msg_code]);
 ...
 */
jint
compress_jpeg(BYTE *data, int width, int height, int quality, jboolean optimize, char *file_name) {
...
Java_com_noblel_baselibrary_utils_ImageUtil_jpegCompressBitmap(JNIEnv *env, jclass type,
                                                             jobject bitmap,
...
JNIEXPORT jint JNICALL
Java_com_noblel_baselibrary_utils_ImageUtil_jpegCompressBitmap2(JNIEnv *env, jclass type,
                                                              jobject bitmap,
